...
def get_directors(search_name: str or None, search_nationality: str or None):
...
def get_top_actor():
    top_actor = Actor.objects.prefetch_related('starring_actor_movie').annotate(
        movie_count=Count('starring_actor_movie'),
        avg_rating=Avg('starring_actor_movie__rating')
    ).order_by('-movie_count', 'full_name').first()

    if not top_actor or not top_actor.movie_count:
        return ''

    movies = ', '.join(m.title for m in top_actor.starring_actor_movie.all())
...
def get_actors_by_movies_count():
    actors = Actor.objects.annotate(movies_count=Count('actors_movie')).order_by(
